...
def main():
    ...
    # Select f_list file, default is f_list.txt
    while True:
    ...
    f_listFile = open(f_listName, "r")
    inputFile = open(inputName, "r")
    outputFile = open(outputName, "w")
    outputFile.write(
        "# fault sim result \n# input: " + cktFile + " \n# input: " + f_listName + " \n# input: " + inputName + "\n\n")

    # Runs the fault parsing for each line of the f_list file
    f_list = []
    for line_fault in f_listFile:
    ...
    for line in inputFile:
        ...
        for line_fault in f_list:
            ...
            if (output != output_fault):
                if (line_fault[1][1] == "IN"):
                    outputFile.write(
                        line_fault[1][0] + "-" + line_fault[1][1] + "-" + line_fault[1][2] + "-" + line_fault[1][
                            3] + "-" + line_fault[1][4] + ": ")
    ...
    for line_fault in f_list:
        if (line_fault[0] == False):
            if (line_fault[1][1] == "IN"):
                outputFile.write(
                    line_fault[1][0] + "-" + line_fault[1][1] + "-" + line_fault[1][2] + "-" + line_fault[1][3] + "-" +
                    line_fault[1][4] + "\n")

            elif (line_fault[1][1] == "SA"):
                outputFile.write(line_fault[1][0] + "-" + line_fault[1][1] + "-" + line_fault[1][2] + "\n")

    outputFile.write("\nfault coverage: " + str(detected_faults) + "/" + str(num_faults) + " = " + "{:.0%}".format(
        detected_faults / num_faults))
    outputFile.close()
